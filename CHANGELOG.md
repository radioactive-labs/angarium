...
(`0.y.0`) may include breaking changes; patch releases (`0.y.z`) stay backward
compatible.
...
- Optional JSON API (mount `Angarium::Engine`): endpoints CRUD +
  ...
  scaffold with `bin/rails g angarium:policy`): `#scope(relation)` (visibility),
  `#owner` (owner of a new endpoint, for admin on-behalf-of), and `#<action>?`
  predicates. The signing secret is revealed only on create/rotate; `custom_headers`
  is write-only.
...
- ActiveJob-based delivery with retries and exponential backoff. Default
  `retry_schedule` follows the Standard Webhooks recommendation of a multi-day
  schedule with exponential backoff: twelve retries over ~10 days
  (`5s, 5m, 30m, 2h, 5h, 10h, 14h, 20h, 24h, 36h, 48h, 72h`).
